...
impl StateMachine {
    ...
    pub fn apply(&self, index: u64, command: &Command) -> Result<Option<String>> {
        let mut data = self.data.write().map_err(|_| {
            RaftError::StorageError("Poisoned lock in state machine".to_string())
        })?;
    ...
    pub fn last_applied_index(&self) -> Result<u64> {
    ...
    pub fn restore_from_snapshot(&self, snapshot: &Snapshot) -> Result<()> {
        ...
        while offset + 8 <= bytes.len() {
            ...
            let key = String::from_utf8(bytes[offset..offset + key_len].to_vec()).map_err(|e| {
                RaftError::StorageError(format!("Invalid UTF-8 in key: {}", e))
            })?;
            offset += key_len;
            ...
            let value = String::from_utf8(bytes[offset..offset + val_len].to_vec()).map_err(|e| {
                RaftError::StorageError(format!("Invalid UTF-8 in value: {}", e))
            })?;
            offset += val_len;
